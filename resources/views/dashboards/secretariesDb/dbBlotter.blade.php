...
@section('style')
    <link rel="stylesheet" href="/css/dbBlotter.css">
            ...
            <div class="mainContentCon">
                <div class="innerContent" id="innerContent">
                    <div class="resContentHeader">
                        <div class="navTitle">BLOTTER ISSUANCE</div>
                        <button class="addResident" id="addRes" onclick="openComplaintForm()">+ BLOTTER</button>
                    </div>
                            ...
                            <tbody>
                                @foreach($complaint as $complaints)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-primary btnview" onclick='viewResCert({{ json_encode($complaints) }})'>View</button>
                                            @if($complaints->blotter_status === 'processed')
                                                <button type="button" class="btn btn-success" onclick="updateBclStatus({{ $complaints->blotter_id }}, 'ready to pick up')">Pickup-Ready</button>
                                            @elseif($complaints->blotter_status !== 'ready to pick up' && $complaints->blotter_status !== 'rejected' && $complaints->blotter_status !== 'cancelled')
                                                <button type="button" class="btn btn-success" onclick="redirectToPurpose({{ $complaints->blotter_id }})">Print</button>
                                                <button type="button" class="btn btn-danger" onclick="showRejectForm({{ $complaints->blotter_id }})">Reject</button>
                                            @elseif($complaints->blotter_status == 'ready to pick up')
                                                <button type="button" class="btn btn-success btnview" onclick="sendEmail('{{ $complaints->res_email }}', '{{ $complaints->res_fname }}', '{{ $complaints->res_lname }}')">Send</button>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
            ...
            <div class="e_rightInput">
                ...
                <div class="e_occupationPart">
                    <div class="complaint">
                        ...
                        <input type="text" class="form-control" name="complaint" id="complaint" placeholder="Enter occupation">
                        <span class="text-danger error-text occupation_error"></span>
                    </div>
                </div>
...
function viewResCert(certificate) {
    ...
    fetch(`/residentComplaint/${resId}`)
        ...
        .then(data => {
            ...
            if (data.blotter_complaint) {
                document.getElementById('complaint').value = data.blotter_complaint;
            } else {
                document.getElementById('complaint').value = 'No complaint';
            }
...
function sendEmail(email, firstName, lastName) {
            ...
            var body = encodeURIComponent(`Good Day! ${firstName} ${lastName},\n\nThis is Barangay Ward II. We would like you to know that your request or issuance has been printed and is ready to pick up we would like you to come at the barangay as soon as possible. Please prepare the necessaries for this complaint you submitted.\n\nThank you. \n\n\n PLEASE DO NOT REPLY!`);
