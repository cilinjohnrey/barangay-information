...
@section('style')
    <link rel="stylesheet" href="/css/dbCert.css">
...
<div class="container">
            ...
            <div class="mainContentCon">
                <div class="innerContent" id="innerContent">
                    <div class="resContentHeader">
                        <div class="navTitle">CERTIFICATE ISSUANCE</div>
                        <button class="addResident" id="addRes" onclick="openCertificateForm()">+ CERTIFICATE</button>
                    </div>
                            ...
                            <tbody>
                                @foreach($certificates as $certificate)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-primary btnview" onclick='viewResCert({{ json_encode($certificate) }})'>View</button>
                                            @if($certificate->certStatus === 'processed')
                                                <button type="button" class="btn btn-success" onclick="updateCertStatus({{ $certificate->id }}, 'ready to pick up')">Pickup-Ready</button>
                                            @elseif($certificate->certStatus !== 'ready to pick up' && $certificate->certStatus !== 'rejected' && $certificate->certStatus !== 'cancelled')
                                                <button type="button" class="btn btn-success" onclick="showPurposeContainer({{ $certificate->id }})">Print</button>
                                                <button type="button" class="btn btn-danger" onclick="showRejectForm({{ $certificate->id }})">Reject</button>
                                            @elseif($certificate->certStatus == 'ready to pick up')
                                                <button type="button" class="btn btn-success btnview" onclick="sendEmail('{{ $certificate->res_email }}', '{{ $certificate->res_fname }}', '{{ $certificate->res_lname }}')">Send</button>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
    ...
    <div class="purposeContainer" style="display: none;">
        <div class="selectPurposeCon">
            <div class="headerPurpose">
                <input type="hidden" id="certificateId" value="" readonly>
                <div class="purposeTitle">SELECT PURPOSE</div>
                <div class="closePurpose"><i class='bx bx-x' onclick="closePurposeContainer()"></i></div>
            </div>
    
            <div class="selectPurpose">
                <div class="purpose1" onclick="redirectToPurpose('Indigency')">Indigency</div>
                <div class="purpose1" onclick="redirectToPurposeBrgyCertification('Barangay Certification')">Barangay Certification</div>
                <div class="purpose1" onclick="redirectToPurposeCertification('Certification')">Certification</div>
                <div class="purpose1" onclick="redirectToPurposegoodMoral('Good Moral')">Good Moral</div>
            </div>
        </div>
    </div>
    ...
    <script>
       document.addEventListener('DOMContentLoaded', function() {
            const purposeContainer = document.querySelector('.purposeContainer');
            const purposeInput = document.getElementById('certificateId');
            const closePurposeButton = purposeContainer.querySelector('.closePurpose i');
            ...
            window.closePurposeContainer = function() {
            ...
            closePurposeButton.addEventListener('click', closePurposeContainer);
        });
    </script>
