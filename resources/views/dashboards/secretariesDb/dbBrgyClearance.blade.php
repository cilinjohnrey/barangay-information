...
@section('style')
    <link rel="stylesheet" href="/css/dbBrgyClearance.css">
        ...
        <div class="sideBar">
            ...
            <div class="navBar">
                ...
                <a href="{{ action('App\Http\Controllers\regValidation@barangayClearance') }}"><div class="resRecord">
                    <button class="btnResRecord act"><span class="resR"> <i class='bx bx-file'></i>Barangay Clearance</span></button>
                </div></a>
            ...
            <div class="mainContentCon">
                <div class="innerContent" id="innerContent">
                    <div class="resContentHeader">
                        <div class="navTitle">BARANGAY CLEARANCE ISSUANCE</div>
                        <button class="addResident" id="addRes" onclick="openBrgyMultiClearance()">+ CLEARANCE</button>
                    </div>
                            ...
                            <tbody>
                                @foreach($clearance as $clearances)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <button type="button" class="btn btn-primary btnview" onclick='viewResCert({{ json_encode($clearances) }})'>View</button>
                                            @if($clearances->bcl_status === 'processed')
                                                <button type="button" class="btn btn-success" onclick="updateBclStatus({{ $clearances->bcl_id }}, 'ready to pick up')">Pickup-Ready</button>
                                            @elseif($clearances->bcl_status !== 'ready to pick up' && $clearances->bcl_status !== 'rejected' && $clearances->bcl_status !== 'cancelled')
                                                <button type="button" class="btn btn-success" onclick="redirectToPurpose({{ $clearances->bcl_id }})">Print</button>
                                                <button type="button" class="btn btn-danger" onclick="showRejectForm({{ $clearances->bcl_id }})">Reject</button>
                                            @elseif($clearances->bcl_status == 'ready to pick up')
                                                <button type="button" class="btn btn-success btnview" onclick="sendEmail('{{ $clearances->res_email }}', '{{ $clearances->res_fname }}', '{{ $clearances->res_lname }}')">Send</button>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                ...
                <div class="emp_leftInput">    
                    <div class="emp_avatarcon">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control picpic" id="emp_profile" name="emp_picture" aria-describedby="inputGroupFileediton03" aria-label="Upload">
                            <span class="text-danger error-text profile_error"></span>
                        </div>  
                    </div>
                </div>
